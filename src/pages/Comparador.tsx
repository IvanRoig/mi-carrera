...
export function Comparador() {
  ...
  const [night, setNight] = useState(settings.maxNightSlots);
  ...
  const rows: Row[] = useMemo(() => {
    function compute(
      ...
      const heavyPerTerm = s.terms.map(
        (t) => t.subjects.filter((c) => (graph.byCode.get(c)?.totalHours ?? 0) > 64).length,
      );
      return {
        id,
        name: label,
        maxNightSlots: over.maxNightSlots ?? settings.maxNightSlots,
        makespan: s.makespan,
        graduation: s.graduation,
        avgLoad: s.makespan ? d.pending.size / s.makespan : 0,
        heavyMax: heavyPerTerm.length ? Math.max(...heavyPerTerm) : 0,
    ...
    return [
      compute('current', `Actual (${settings.maxNightSlots}/cuatri)`, {}, true),
      ...scenarios.map((sc) =>
        compute(sc.id, sc.name, {
          maxNightSlots: sc.maxNightSlots,
          maxNonNightSlots: sc.maxNonNightSlots,
        }),
      ),
    ];
  }, [d.pending, d.done, settings, scenarios]);
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-slate-600 dark:text-slate-400">
        Compará distintas estrategias (p.ej. “6 por cuatri” vs “5 por cuatri”)
        para ver cómo cambian la fecha de egreso, la carga y el riesgo. El{' '}
        <strong>riesgo</strong> es la cantidad de materias en la ruta crítica:
        cuantas más, menos margen tenés si se cae una comisión.
      </p>
      ...
      <div className="flex flex-wrap items-end gap-3 rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
        <label className="flex flex-col gap-1">
          ...
          <input
            ...
            placeholder="Ej: 6 por cuatri a full"
            className="w-52 rounded-lg border border-slate-300 bg-transparent px-3 py-1.5 text-sm dark:border-slate-700"
          />
        </label>
        <label className="flex flex-col gap-1">
          <span className="text-xs text-slate-500 dark:text-slate-400">Materias/noche</span>
          <input
            type="number"
            min={1}
            max={10}
            value={night}
            onChange={(e) => setNight(Math.max(1, +e.target.value || 1))}
            className="w-24 rounded-lg border border-slate-300 bg-transparent px-3 py-1.5 text-sm dark:border-slate-700 dark:bg-slate-900"
          />
        </label>
        <button
          onClick={() => {
            addScenario({
              name: name.trim() || `${night} por cuatri`,
              maxNightSlots: night,
              maxNonNightSlots: settings.maxNonNightSlots,
            });
            setName('');
          }}
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 text-left text-xs uppercase text-slate-500 dark:bg-slate-900 dark:text-slate-400">
            <tr>
              ...
              <th className="px-4 py-2 font-medium">Máx. pesadas/cuatri</th>
              <th className="px-4 py-2 font-medium">Riesgo (ruta crítica)</th>
          ...
          <tbody>
            {rows.map((r) => (
              ...
              >
                ...
                <td className="px-4 py-2">{r.heavyMax}</td>
